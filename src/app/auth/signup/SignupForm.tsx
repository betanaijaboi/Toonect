...
function SignupInner() {
  ...
  // Auto-open on arrival at the details step (deep link via ?role=), and again from the
  // "Continue" button below — both are event-driven, not effect-driven, per react-hooks/set-state-in-effect.
  const [showTermsModal, setShowTermsModal] = useState(() => !!urlRole);
  const [termsAcceptedAt, setTermsAcceptedAt] = useState<string | null>(null);

  // Keep role in sync with a ?role= URL change without a full remount (e.g. clicking
  // "Join as Artist" while already on this page) — adjusted during render, not in an effect.
  ...
  function goToDetails() {
    setStep("details");
    if (!termsAcceptedAt) setShowTermsModal(true);
  }
        ...
        {step === "details" && (
          <div className="flex flex-col gap-6">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              ...
              <input type="hidden" name="terms_accepted_at" value={termsAcceptedAt ?? ""} />
              ...
              <button
                ...
                className={clsx(
                  "flex items-center gap-2 text-xs rounded-xl px-3 py-2.5 border text-left transition-colors",
                  termsAcceptedAt
                    ? "border-green-200 bg-green-50 text-green-700"
                    : "border-[var(--border)] text-[var(--muted)] hover:border-[var(--accent)] hover:text-[var(--accent)]"
                )}
              >
                {termsAcceptedAt ? <CheckCircle className="w-4 h-4 flex-shrink-0" /> : null}
                {termsAcceptedAt
              ...
              <button
                type="submit"
                disabled={pending || !termsAcceptedAt}
